...
def get_all_pdfs(dir_path):
    ...
    if not os.path.exists('./pdfs.csv'):
        ...
        for file_dir in os.listdir(dir_path):
            pdf_dir = os.path.join(dir_path, file_dir, 'pdf')
            for file in os.listdir(pdf_dir):
                if file.endswith('.pdf'):
                    pdfs.append(os.path.join(pdf_dir, file).replace("\\", "/"))
                    pdf_dirs.append([file_dir, file])
...
def get_pic_from_pdf(pdf_path,pages):
    ...
    # 保存图片
    images_path = []
    if not os.path.exists(pdf_dir + '/images'):
        os.makedirs(pdf_dir + '/images')
...
def extract_table_text(api_key,images_path):

    if len(images_path) > 0:
        print("Call GPT-4 Vision API...")
        answers_csv = []
